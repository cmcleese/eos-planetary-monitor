...
// Pure utility — no component state or closure deps, lives outside the component
...
export default function ISSLayer() {
  ...
  // Lazy ref initialization — configure the position property once without an extra useEffect
  ...
  if (!positionPropertyRef.current) {
    ...
    positionPropertyRef.current = prop;
  }
  ...
  useEffect(() => {
    ...
    const ds = new CustomDataSource('iss-layer');
    ...
    // Created once inside the effect — stable for the lifetime of the layer.
    const dynamicLabelOffset = new CallbackProperty((time, result) => {
      ...
      const distance = Cartesian3.distance(viewer.camera.position, entityPos);
    ...
    const trackISS = (lat: number, lon: number, alt: number) => {
      ...
      viewer.clock.currentTime = JulianDate.addSeconds(currentTime, -10, new JulianDate());
    ...
    const interval = window.setInterval(fetchISSPosition, 5000);

    // Cleanup on unmount remove the DataSource and all its entities
    return () => {
